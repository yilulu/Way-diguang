...
(function($) {
    ...
    $.ligerDefaults.Dialog = {
        ...
        isDrag: true,   //是否拖動
        width: 280,     //寬度
        height: null,   //高度，默認自我調整 
        content: '',    //內容
        target: null,   //目標物件，指定它將以appendTo()的方式載入
        url: null,      //目標頁url，預設以iframe的方式載入
        load: false,     //是否以load()的方式載入目標頁的內容
        type: 'warn',   //類型 warn、success、error、question
        left: null,     //位置left
        top: null,      //位置top
        modal: true,    //是否模態對話方塊
    ...
    $.ligerDialog.alert = function(content, title, type, callback) {
        ...
        p = {
            content: content,
            buttons: [{ text: '確定', onclick: btnclick}]
        };
    ...
    $.ligerDialog.prompt = function(title,value,multi, callback) {
        ...
        p = {
            ...
            target: target,
            width:320,
            buttons: [{ text: '確定', onclick: btnclick, type: 'yes' }, { text: '取消', onclick: btnclick, type: 'cancel'}]
        };
        $.ligerDialog.open(p);
    };
...
(function ($) {
    ...
    $.ligerDefaults.Layout = {
        ...
        centerWidth: 300,
        rightWidth: 170,
        InWindow : true,     //是否以視窗的高度為准 height設置為百分比時可用
        heightDiff : 0,     //高度補差
        height:'100%',      //高度
        ...
        return this.each(function ()
        {
            ...
            var g = {
                ...
                start: function (e, dir)
                {
                    ...
                    if ($.browser.msie || $.browser.safari) 
                        $('body').bind('selectstart', function () { return false; }); // 不能選擇
...
(function($) {
    ...
    $.ligerDefaults.Tip = {
        ...
        distanceX: 7,
        distanceY: 0,
        appendIdTo: null       //保存ID到那一個物件(jQuery)
    };
    ...
    $.fn.ligerTree = function (p)
    {
        this.each(function ()
        {
            ...
            var po = { 
                ...
                convertData:function(data)      //將ID、ParentID這種資料格式轉換為樹格式
                {
                    if(!data || !data.length) return []; 
                    var isolate = function(pid)//根據ParentID判斷是否孤立
                    {
                    ...
                    var targetData = [];                    //存儲資料的容器(返回)
                    var itemLength = data.length;           //資料集合的個數
                    var insertedLength = 0;                 //已插入的資料個數
                    var currentIndex = 0;                   //當前資料索引
                    var getItem = function(container,id)    //獲取資料項目(為空時表示沒有插入)
                    {
                    ...
                    var addItem = function(container,item)  //插入資料項目
                    {
                        container.push($.extend({},item));
                        insertedLength++;
                    };
                    //判斷已經插入的節點和孤立節點 的個數總和是否已經滿足條件
                    ...
                    { 
                        ...
                        else
                        {
                            ...
                            if(pitem && getItem(targetData,id) == null)//找到父節點資料並且還沒插入
                            {  
