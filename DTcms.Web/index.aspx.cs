...
(select id from dt_article where channel_id=" + mid + " and Status=3))";
        ...
        public void Bind1()
        {
            repdateNew.DataSource = dalArticle.GetPageindexList("", 2, " and Status=1 and Postid=6");  //兩條新房推薦數據
            repdateNew.DataBind();

            var table1 = dalArticle.GetPageindexList("1", 8, " and Status=1 and (channel_id=3 or  channel_id=2)").Tables[0];
            for (int i = 0; i < table1.Rows.Count; i++)
            {
                if (i < 6)
                {
        ...
        {
            //不動產出租
            repdatepost2.DataSource = dalArticle.GetPageindexList("", 5, " and Status=1 and channel_id=3");
            repdatepost2.DataBind();

            //不動產出售
            repdatepost3.DataSource = dalArticle.GetPageindexList("", 5, " and Status=1 and channel_id=2");
        ...
        public void Bind3()
        {
            var table = dalArticle.GetPageindexList("", 13, " and Status=1 and channel_id=4").Tables[0];
            for (int i = 0; i < table.Rows.Count; i++)
            {
                if (i < 6)
                {
        ...
        public void Bind4()
        {
            var table = dalArticle.GetPageindexList("", 9, " and Status=1 and channel_id=5").Tables[0];
            for (int i = 0; i < table.Rows.Count; i++)
            {
                if (i == 0)
                {
