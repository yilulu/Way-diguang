...
namespace DTcms.Web.admin.users
{
    public partial class user_config : Web.UI.ManagePage
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!Page.IsPostBack)
            {
                ChkAdminLevel("user_config", DTEnums.ActionEnum.View.ToString()); //檢查許可權
                ShowInfo();
            }
        }
        ...
        {
            ChkAdminLevel("user_config", DTEnums.ActionEnum.Edit.ToString()); //檢查許可權
            BLL.userconfig bll = new BLL.userconfig();
            Model.userconfig model = bll.loadConfig(Utils.GetXmlMapPath(DTKeys.FILE_USER_XML_CONFING));
            try
            {
                ...
                JscriptMsg("修改用戶配置成功！", "user_config.aspx", "Success");
            }
            catch
            {
