...
namespace DTcms.Web.admin.comment
{
    public partial class edit : Web.UI.ManagePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            {
                JscriptMsg("訊息不存在或已被刪除！", "back", "Error");
                return;
            }
            if (!Page.IsPostBack)
            {
        ...
        {
            ChkAdminLevel("sys_comment", DTEnums.ActionEnum.View.ToString()); //檢查權限
        ...
        {
            ChkAdminLevel("sys_comment", DTEnums.ActionEnum.Reply.ToString()); //檢查權限
            BLL.article_comment bll = new BLL.article_comment();
            ...
            bll.Update(model);
            JscriptMsg("物件提問回覆成功！", "list.aspx", "Success");
        }
    }
}