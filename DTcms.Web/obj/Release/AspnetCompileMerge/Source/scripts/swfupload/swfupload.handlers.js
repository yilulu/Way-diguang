...
function addImage(bigSrc, smallSrc) {
    ...
    var newImg = $('<div onclick="focus_img(this);" class=\"img_box ' + claname + '">' +
        ...
        '<span class="remark"><i>暫無描述...</i></span></div>').appendTo(newLi);
    var forea = $('<a href="javascript:;" onclick="show_remark(this);">描述</a>').appendTo(newLi);
    var downa = $('<a href="javascript:;" onclick="del_img(this);">刪除</a>').appendTo(newLi);
...
function del_img(obj) {
    ...
    var node = $(obj).parent(); //要刪除的LI節點
    node.remove(); //刪除DOM元素
    //檢查是否為封面
    if (focusphoto.val() == smallimg) {
        focusphoto.val("");
        var firtimg = $("#show_list ul li .img_box img").eq(0);
        firtimg.parent().addClass("current"); //取第一張做為封面
        focusphoto.val(firtimg.attr("src")); //重新給封面的隱藏欄位賦值
    }
}
...
function show_remark(obj) {
    ...
    var m = $.ligerDialog.open({
        ...
        width: 350,
        buttons: [
        { text: '批次描述', onclick: function () {
            var remark = $('#ImageRemark').val();
            if (remark == "") {
                $.ligerDialog.error('總該寫點什麽吧？');
                return false;
            }
...
function fileQueueError(file, errorCode, message) {
	try {
		...
		case SWFUpload.QUEUE_ERROR.INVALID_FILETYPE:
			progress.setStatus(file.name+"文件類型出錯");
			progress.toggleCancel(false, this);
			break;
		default:
			if (file !== null) {
				progress.setStatus("未知錯誤");
				progress.toggleCancel(false, this);
			}
			break;
		}
...
function uploadProgress(file, bytesLoaded) {
	...
	try {
		...
		if (percent === 100) {
			progress.setStatus("創建縮略圖...");
			progress.toggleCancel(false, this);
		} else {
			progress.setStatus("上傳...");
