...
miniWin = function() {
    this.options = function(opt) {
        ...
        back = {
            ...
            showType: opt.showType || "fade"			//背景展示方式[show:直接显示或關閉，fade:淡入淡出，设置为fade时请将filter值设置大一点] 
        };
        miniBox = {
            skinPath: opt.skinPath || "http://www.empro.com.tw/images/miniWin.default.gif", //皮肤路径
            ...
            auto: opt.auto == null ? false : opt.auto, 		//是否自动關閉窗口
            autoTimes: opt.autoTimes || 3, 				//自动關閉时间,秒
            isBlink: opt.isBlink == null ? true : opt.isBlink,
            show: opt.show || "show", 					//窗口關閉或打开方式[win7:模拟win7效果,fade:淡入淡出,show:直接打开]
            ...
            loaderImg: opt.loaderImg || "http://www.empro.com.tw/images/loading.gif" //loadingUrl，请根据实际设置该地址
        };
    };
    //相关常量的值
    ...
    this.appendMini = function() {
        ...
        if (miniBox.auto) {
            ...
            HQBox += " title=\"關閉\" id=\"" + miniID + "_Closer\" onclick=HQcallback(" + obj2Str(setting) + ",\"closeMini()\")></span>";
        }
        HQBox += "</div>";
        //内容
