...
namespace DTcms.DAL
{
    ...
    public partial class amount_log
    {
        ...
        public DataTable list_pagesWhere(int page, int numPerPage, string whereStr, string orderStr)
        {
            ...
            try
            {
                sql = "select top " + numPerPage + " * from dt_amount_log where 1=1";
                string sql1 = "";
                if (orderStr == "")
                {
                ...
                if (page > 1)
                {
                    sql += " and id not in(select top " + (page - 1) * numPerPage + " id from dt_amount_log where 1=1 " + sql1 + orderStr + ")";
                }
                sql += orderStr;
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select count(id) FROM dt_amount_log");
