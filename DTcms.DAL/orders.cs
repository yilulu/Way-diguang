...
namespace DTcms.DAL
{
    ...
    public partial class orders
    {
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select  top 1 id,order_no,user_id,user_name,payment_id,distribution_id,status,payment_status,distribution_status,delivery_name,delivery_no,accept_name,post_code,telphone,mobile,address,message,payable_amount,real_amount,payable_freight,real_freight,payment_fee,order_amount,point,add_time,payment_time,confirm_time,distribution_time,complete_time,order_amount from dt_orders ");
            strSql.Append(" where id=@id");
            SqlParameter[] parameters = {
					new SqlParameter("@id", SqlDbType.Int,4)};
            parameters[0].Value = id;
        ...
        public DataTable list_pagesWhere(int page, int numPerPage, string whereStr, string orderStr)
        {
            ...
            try
            {
                sql = "select top " + numPerPage + " * from dt_orders where 1=1";
                string sql1 = "";
                if (orderStr == "")
                {
                ...
                if (page > 1)
                {
                    sql += " and id not in(select top " + (page - 1) * numPerPage + " id from dt_orders where 1=1 " + sql1 + orderStr + ")";
                }
                sql += orderStr;
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select count(id) FROM dt_orders");
        ...
        {
            int bk = 0;
            if (!string.IsNullOrEmpty(orNo))
            {
                StringBuilder strSQL = new StringBuilder();
                strSQL.Append("UPDATE dt_orders set payment_status=3 where order_no='" + orNo + "'");
