...
namespace DTcms.DAL
{
    ...
    public partial class article
    {
        ...
        public int Add(Model.article_news model)
        {
            ...
            strSql.Append("channel_id,category_id,title,link_url,img_url,seo_title,seo_keywords,seo_description,content,sort_id,click,is_lock,user_id,add_time,Status)");
        ...
        /// 得到一个对象实体
        /// </summary>
        public Model.article_news GetModel(int id)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select  top 1 * from dt_article ");
            strSql.Append(" where id=@id ");
            SqlParameter[] parameters = {
					new SqlParameter("@id", SqlDbType.Int,4)};
            parameters[0].Value = id;
        ...
        public DataTable list_page(int page, int numPerPage, string whereStr, string orderStr)
        {
            ...
            try
            {
                sql = "select top " + numPerPage + " * from view_article_news where 1=1";
                string sql1 = "";
                if (orderStr == "")
                {
                ...
                if (page > 1)
                {
                    sql += " and id not in(select top " + (page - 1) * numPerPage + " id from view_article_news where 1=1 " + sql1 + orderStr + ")";
                }
                sql += orderStr;
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select count(id) FROM view_article_news");
