...
namespace DTcms.DAL
{
    ...
    public partial class manager
    {
        ...
        /// 根据用户名密码返回一个实体
        /// </summary>
        public Model.manager GetModelVIP(string user_name, string user_pwd)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select id from dt_manager");
            strSql.Append(" where user_name=@user_name and user_pwd=@user_pwd and is_lock=0 AND role_id=4 ");
            SqlParameter[] parameters = {
					new SqlParameter("@user_name", SqlDbType.NVarChar,100),
                    new SqlParameter("@user_pwd", SqlDbType.NVarChar,100)};
            parameters[0].Value = user_name;
