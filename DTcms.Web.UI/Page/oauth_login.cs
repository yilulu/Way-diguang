...
namespace DTcms.Web.UI.Page
{
    public partial class oauth_login : Web.UI.BasePage
    {
        ...
        void UserPage_Init(object sender, EventArgs e)
        {
            ...
            {
                HttpContext.Current.Response.Redirect(config.webpath + "error.aspx?msg=" + Utils.UrlEncode("登入失敗，用戶授權已過期，請重新登入！"));
                return;
            }
            ...
            if (oauthModel != null)
            {
                ...
                if (model == null)
                {
                    HttpContext.Current.Response.Redirect(config.webpath + "error.aspx?msg=" + Utils.UrlEncode("登入失敗，授權用戶不存在或已被刪除！"));
                    return;
                }
