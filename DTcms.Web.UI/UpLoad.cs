...
namespace DTcms.Web.UI
{
    public class UpLoad
    {
        ...
        public string fileSaveAs(HttpPostedFile postedFile, bool isThumbnail, bool isWater, bool IsKind)
        {
            return fileSaveAs(postedFile, isThumbnail, isWater, false, false, IsKind);
        }
        ...
        /// <returns>服务器文件路径</returns>
        public string fileSaveAs(HttpPostedFile postedFile, bool isThumbnail, bool isWater, bool _isImage, bool IsKind)
        {
            return fileSaveAs(postedFile, isThumbnail, isWater, _isImage, false, IsKind);
        }
        ...
        /// <returns>服务器文件路径</returns>
        public string fileSaveAs(HttpPostedFile postedFile, bool isThumbnail, bool isWater, bool _isImage, bool _isReOriginal, bool IsKind)
        {
            try
            {
                ...
                {
                    return "{\"msg\": 0, \"msgbox\": \"不允許上傳" + fileExt + "類型的文件！\"}";
                }
                //检查是否必须上传图片
                if (_isImage && !IsImage(fileExt))
                {
                    return "{\"msg\": 0, \"msgbox\": \"對不起，僅允許上傳圖片文件！\"}";
                }
                //检查文件大小是否合法
                if (!CheckFileSize(fileExt, postedFile.ContentLength))
                {
                    return "{\"msg\": 0, \"msgbox\": \"文件超過限制的大小啦！\"}";
                }
                //获得要保存的文件路径
                string serverFileName = dirPath + fileName;
                string url = HttpContext.Current.Request.Url.Host;
                string serverThumbnailFileName = "http://" + url + dirPath + "small_" + fileName;
                ...
                if (IsKind)
                {
                    if (IsImage(fileExt) && (this.siteConfig.kindmaxheight > 0 || this.siteConfig.kindgmaxwidth > 0))
                    {
                        Thumbnail.MakeThumbnailImage(toFileFullPath + fileName, toFileFullPath + fileName, this.siteConfig.kindgmaxwidth, this.siteConfig.kindmaxheight);
                    }
                }
                else
                {
            ...
            {
                return "{\"msg\": 0, \"msgbox\": \"上傳過程中發生意外錯誤！\"}";
            }
        }
